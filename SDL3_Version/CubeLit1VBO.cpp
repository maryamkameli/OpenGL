...
//4 - A cube lit with abmient and diffuse lighting (SDL3 version)
...
//Tell OpenGL how to set shader inputs
...
// The normal data starts after the vertex data
// Vertex data size: 36 vertices * 8 floats/vertex * sizeof(float)
...
glUseProgram(shaderProgram); //Set the active shader (only one can be used at a time)
    ...
    while (SDL_PollEvent(&windowEvent)){
		...
		if (windowEvent.type == SDL_EVENT_KEY_UP && windowEvent.key.key == SDLK_ESCAPE)
			quit = true;
		if (windowEvent.type == SDL_EVENT_KEY_UP && windowEvent.key.key == SDLK_F) { // Toggle fullscreen
